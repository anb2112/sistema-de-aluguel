...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $id_f = filter_input(INPUT_POST, 'id_f');
    ...
    $forma_pag  = filter_input(INPUT_POST, 'forma_pag');
    $status_pg  = filter_input(INPUT_POST, 'status_pg');
    $data_in  = filter_input(INPUT_POST, 'data_in');
    $data_f   = filter_input(INPUT_POST, 'data_f');
    $placa_v   = filter_input(INPUT_POST, 'placa_v');
    $id_v   = filter_input(INPUT_POST, 'id_v');
    $modelo   = filter_input(INPUT_POST, 'modelo');
    $nome_v   = filter_input(INPUT_POST, 'nome_v');
...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "save" && $id_f != "") {
    try {
        if ($id != "") {
            $stmt = $conexao->prepare("UPDATE aluguel SET  id_f=?, nome_f=?, nome=?, telefone=?, cpf=?,rg=?, data_nascimento=?, email=?, sexo=?, cnh=?, cep=?, endereço=?,cidade=?,bairro=?, numero=?, complemento=?, forma_pag=?, status_pg=?, data_in=?, data_f=?, placa_v=?, id_v=?, modelo=?, nome_v=?, fabricante=? WHERE id = ?");
            $stmt->bindParam(26, $id);
        } else {
            $stmt = $conexao->prepare("INSERT INTO aluguel ( id_f, nome_f, nome, telefone, cpf, rg, data_nascimento, email, sexo, cnh, cep, endereço, cidade, bairro, numero, complemento, forma_pag, `status_pg, data_in, data_f, placa_v, id_v, modelo, nome_v, fabricante) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        }
        $stmt->bindParam(1, $id_f);
        $stmt->bindParam(2, $nome_f);
        ...
        $stmt->bindParam(10, $cnh );
        $stmt->bindParam(11, $cep );
        $stmt->bindParam(12, $endereço );
        $stmt->bindParam(13, $cidade );
        $stmt->bindParam(14, $bairro );
        $stmt->bindParam(15, $numero );
        $stmt->bindParam(16, $complemento );
        $stmt->bindParam(17, $forma_pag );
        $stmt->bindParam(18, $status_pg );
        $stmt->bindParam(19, $data_in );
        $stmt->bindParam(20, $data_f );
        $stmt->bindParam(21, $placa_v );
        $stmt->bindParam(22, $id_v );
        $stmt->bindParam(23, $modelo );
        $stmt->bindParam(24, $nome_v  );
        $stmt->bindParam(25, $fabricante  );
...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "upd" && $id != "") {
    try {
        $stmt = $conexao->prepare("SELECT * FROM aluguel WHERE id = ?");
        ...
        if ($stmt->execute()) {
            ...
            $forma_pag = $rs->forma_pag;
            $status_pg = $rs->status_pg;
            $data_in = $rs->data_in;
            $data_f = $rs->data_f;
            $placa_v = $rs->placa_v;
            $id_v = $rs->id_v;
...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "del" && $id != "") {
    try {
        $stmt = $conexao->prepare("DELETE FROM aluguel WHERE id = ?");
        $stmt->bindParam(1, $id, PDO::PARAM_INT);
        if ($stmt->execute()) {
                    ...
                    <form action="?act=save" method="POST" name="form1" class="form-horizontal" >
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                <span class="panel-title">CRUD Aluga</span>
                            </div>
                            <div class="panel-body">
                                ...
                                <div class="form-group">
                                    <label for="id_f" class="col-sm-1 control-label">ID Funcionario :</label>
                                    <div class="col-md-5">
                                        <input type="text" name="id_f" value="<?php
                                        echo (isset($id_f) && ($id_f != null || $id_f != "")) ? $id_f : '';

                                        ?>" class="form-control"/>
                                    </div>
                                    <label for="nome_f" class="col-sm-1 control-label">Nome funcionario:</label>
                                    <div class="col-md-4">
                                        <input type="text" name="nome_f" value="<?php
                                        echo (isset($nome_f) && ($nome_f != null || $nome_f != "")) ? $nome_f : '';
                                ...
                                <div class="form-group">
                                    ...
                                    <label for="forma_pag" class="col-sm-4 control-label">Forma de pag:</label>
                                    <div class="col-md-2">
                                        <input type="text" name="forma_pag" value="<?php
                                        echo (isset($forma_pag) && ($forma_pag != null || $forma_pag != "")) ? $forma_pag : '';
                                ...
                                <div class="form-group">
                                    <label for="status_pg" class="col-sm-1 control-label">Status do pagamento:</label>
                                    <div class="col-md-2">
                                        <input type="text" name="status_pg" <?php
                                        echo (isset($status_pg) && ($status_pg != null || $status_pg != "")) ? $status_pg : '';

                                        ?> class="form-control" />
                                    </div>
                                    <label for="data_in" class="col-sm-4 control-label">Data inicial:</label>
                                    <div class="col-md-2">
                                        <input type="date" name="data_in" value="<?php
                                        echo (isset($data_in) && ($data_in != null || $data_in != "")) ? $data_in : '';
                                ...
                                <div class="form-group">
                                    <label for="data_f" class="col-sm-1 control-label">Data final:</label>
                                    <div class="col-md-2">
                                        <input type="date" name="data_f" <?php
                                        echo (isset($data_f) && ($data_f != null || $data_f != "")) ? $data_f : '';

                                        ?> class="form-control" />
                                    </div>
                                    <label for="placa_v" class="col-sm-4 control-label">Placa veiculo:</label>
                                    <div class="col-md-2">
                                        <input type="text" name="placa_v" value="<?php
                                        echo (isset($placa_v) && ($placa_v != null || $placa_v != "")) ? $placa_v : '';
                                ...
                                <div class="form-group">
                                    <label for="id_v" class="col-sm-1 control-label">ID veiculo:</label>
                                    <div class="col-md-2">
                                        <input type="text" name="id_v" <?php
                                        echo (isset($id_v) && ($id_v != null || $id_v != "")) ? $id_v : '';
                                ...
                                <div class="form-group">
                                    <label for="nome_v" class="col-sm-1 control-label">Nome Veiculo:</label>
                                    <div class="col-md-2">
                                        <input type="text" name="nome_v" <?php
                                        echo (isset($nome_v) && ($nome_v != null || $nome_v != "")) ? $nome_v : '';

                                        ?> class="form-control" />
                                    </div>
                                    <label for="fabricante" class="col-sm-4 control-label">Fabricante:</label>
                                    <div class="col-md-2">
                                        <input type="text" name="fabricante" value="<?php
                                        echo (isset($fabricante) && ($fabricante != null || $fabricante != "")) ? $fabricante : '';
                    ...
                    <div class="panel panel-default">
                        ...
                        <thead>
                            ...
                            <tbody>
                                ...
                                try {
                                    $stmt = $conexao->prepare("SELECT * FROM aluguel");
                                    if ($stmt->execute()) {
                                        while ($rs = $stmt->fetch(PDO::FETCH_OBJ)) {

                                            ?><tr>
                                                <td><?php echo $rs->id_f ; ?></td>
                                                <td><?php echo $rs->nome_f; ?></td>
                                                ...
                                                <td><?php echo $rs->forma_pag   ; ?></td>
                                                <td><?php echo $rs->status_pg  ; ?></td>
                                                <td><?php echo $rs->data_in  ; ?></td>
                                                <td><?php echo $rs->data_f  ; ?></td>
                                                <td><?php echo $rs->placa_v  ; ?></td>
                                                <td><?php echo $rs->id_v    ; ?></td>
                                                <td><?php echo $rs->modelo   ; ?></td>
                                                <td><?php echo $rs->nome_v   ; ?></td>
                                                <td><?php echo $rs->fabricante   ; ?></td>
                                                <td><center>
